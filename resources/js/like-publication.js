document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.like-form').forEach(function(form) {
        form.addEventListener('submit', function(e) {
            ...
            const svg = this.querySelector('svg'); // Беремо сам SVG для зміни кольору
            ...
            fetch('/publication/like', {
                ...
                .then(data => {
                    if (data.success) {
                        // 1. Використовуємо булеве значення з сервера (data.liked)
                        if (data.liked) {
                            svgPath.setAttribute('d', likedPath);
                            svgPath.setAttribute('fill', '#EF4444'); // Червоний
                            svg.setAttribute('fill', '#EF4444'); // Про всяк випадок для SVG контейнера
                        } else {
                            svgPath.setAttribute('d', unlikedPath);
                            svgPath.setAttribute('fill', '#9CA3AF'); // Сірий

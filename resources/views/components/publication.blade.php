            ...
            <div class="p-4 border-t border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-between mb-4">
                    <div class="flex items-center space-x-6">
                        <!-- Like button -->
                        <form class="like-form" data-publication-id="{{ $publication->id }}">
                            @csrf
                            <button type="submit" class="group flex items-center space-x-2 transition-all duration-200 hover:scale-105 like-button">
                                @if($publication->is_liked)
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" viewBox="0 -960 960 960" fill="#EF4444">
                                        <path d="m480-120-58-52q-101-91-167-157T150-447.5Q111-500 95.5-544T80-634q0-94 63-157t157-63q52 0 99 22t81 62q34-40 81-62t99-22q94 0 157 63t63 157q0 46-15.5 90T810-447.5Q771-395 705-329T538-172l-58 52Z"/>
                                    </svg>
                                @else
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" viewBox="0 -960 960 960" fill="#9CA3AF">
                        ...
                        <button class="group flex items-center space-x-2 transition-all duration-200 hover:scale-105 focus:outline-none">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-gray-400 dark:text-gray-500 group-hover:text-green-500 dark:group-hover:text-green-400 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <!-- Repost button -->
                    @if($publication->user_id !== auth()->user()->id)
                        <form class="repost-form" data-publication-id="{{ $publication->id }}">
                            @csrf
                            <button type="submit" class="group flex items-center space-x-2 transition-all duration-200 hover:scale-105 like-button">
                                @if($publication->is_reposted)
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-blue-500 dark:text-blue-400" fill="currentColor" viewBox="0 0 24 24">
                ...
                <div class="mt-2">
                    <p class="text-gray-400 dark:text-gray-500 text-xs cursor-pointer hover:text-blue-500 dark:hover:text-blue-400 transition-colors comments-trigger">
                        {{ __('publication.viewComments') }} ({{ $publication->commentsCount }})
                    </p>

                    <!-- Comments Modal -->
                    <div id="comments-popup" class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 hidden p-4">
                        <div class="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden shadow-2xl border border-gray-200 dark:border-gray-700 flex flex-col">
                            ...
                            <div class="flex border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
                                <button id="comments-tab" class="flex items-center justify-center gap-2 py-4 px-6 flex-1 text-gray-900 dark:text-white font-semibold border-b-2 border-blue-500 transition-colors">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/>
                                    </svg>
                                ...
                                <button id="likes-tab" class="flex items-center justify-center gap-2 py-4 px-6 flex-1 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 font-medium transition-colors">
                                ...
                                <button id="close-popup" class="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 p-4 transition-colors">
                                ...
                                <div id="comments-content" class="space-y-4">
                                            ...
                                            <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4 border border-gray-200 dark:border-gray-600">
                                                <div class="flex items-start justify-between">
                                                    ...
                                                    <form class="like-comment-form" data-comment-id="{{ $comment->id }}">
                                                        @csrf
                                                        <button type="submit" class="group flex items-center space-x-1 transition-all duration-200 hover:scale-105 like-button">
                                                            @if($comment->is_liked)
                                    ...
                                    @else
                                        <div class="text-center py-8">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 mx-auto text-gray-300 dark:text-gray-600 mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/>
                                            </svg>
                            ...
                            <!-- Add comment section -->
                            <div id="comment-input-section" class="border-t border-gray-200 dark:border-gray-700 p-4 bg-gray-50 dark:bg-gray-900/50">
                                <form action="{{ route('comment.create') }}" method="post" class="flex items-center space-x-3">
                                    @csrf
                                    @method('put')
                                    ...
                                    <button type="submit" class="px-5 py-2.5 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white text-sm font-semibold rounded-full transition-all duration-200 shadow-sm hover:shadow-md">
                                        {{ __('publication.post') }}
                                    </button>
                                </form>
                            </div>

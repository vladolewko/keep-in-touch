<x-admin-layout>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    @foreach($users as $user)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                {{-- ДОДАНО id="status-badge-{{ $user->id }}" --}}
                                <span id="status-badge-{{ $user->id }}"
                                    class="w-20 px-2 flex justify-center text-xs leading-5 font-semibold rounded-full {{ $user->deleted_at ? 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200' : 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' }}">
        {{ $user->deleted_at ? 'Blocked' : 'Active' }}
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex space-x-1">
                                    <button type="button"
                                        onclick="toggleUserBlock({{ $user->id }}, '{{ route('admin.user.block', ['id' => $user->id]) }}')"
                                        id="btn-block-{{ $user->id }}"
                                        class="w-24 {{ $user->deleted_at ? 'bg-green-500 hover:bg-green-600' : 'bg-red-500 hover:bg-red-600' }} text-white py-1 px-3 rounded-md text-sm font-medium transition-colors duration-150 flex items-center justify-center">

                                        {{-- Icon --}}
                                        <svg id="icon-block-{{ $user->id }}"
                                            ...
                                            <path stroke-linecap="round"
                                                stroke-linejoin="round"
                                                stroke-width="2"
                                                d="{{ $user->deleted_at ? 'M13 10V3L4 14h7v7l9-11h-7z' : 'M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636' }}"/>
                                        </svg>

                                        {{-- Text --}}
                                        <span id="text-block-{{ $user->id }}">
                                            {{ $user->deleted_at ? 'Restore' : 'Block' }}
                                        </span>
                                    </button>
                                    <a href="{{ route('admin.user.message', ['userId' => $user->id]) }}"
    ...
    <script>
        async function toggleUserBlock(userId, url) {
            const btn = document.getElementById(`btn-block-${userId}`);
            const btnText = document.getElementById(`text-block-${userId}`);
            const iconSvg = document.getElementById(`icon-block-${userId}`);
            const iconPath = iconSvg.querySelector('path');
            const statusBadge = document.getElementById(`status-badge-${userId}`);

            const originalBtnClasses = [... btn.classList];
            const originalBadgeClasses = [... statusBadge.classList];
            const originalText = btnText.innerText;
            const originalIcon = iconPath.getAttribute('d');
            ...
            try {
                ...
                if (data.success) {
                    ...
                    if (data.is_blocked) {
                        ...
                        iconPath.setAttribute('d', "M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636");
                    }
                }
                ...
                btn.className = '';
                btn.classList.add(... originalBtnClasses);

                statusBadge.className = '';
                statusBadge.classList.add(... originalBadgeClasses);

                btnText.innerText = originalText;
                iconPath.setAttribute('d', originalIcon);
